...
from models import PVT,wavemlp
...
class PVTBlock(nn.Module):
    def __init__(self, c1, c2, patch_size, mlp_ratio, sr_ratios, depths, n=1):
        ...
        self.conv = None
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.PVTLayer(in_chans=c1, embed_dims=c2, patch_size=patch_size, num_heads=num_heads, 
...
class PVTBlockv2(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.PVTLayerv2(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
...
class CPVTBlock(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.CPVTLayer(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
                                   mlp_ratio=mlp_ratio, sr_ratios=sr_ratios, depths=depths, linear=True) for i in range(n)])
...
class WaveBlock(nn.Module):
    def __init__(self, c1, c2, mlp_ratio, n=1):
        super().__init__()
        self.blocks = nn.Sequential(*[wavemlp.WaveBlock(dim=c1, out=c2, mlp_ratio=mlp_ratio) for i in range(n)])
...
class WavePVT(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.WavePVTLayer(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
                                   mlp_ratio=mlp_ratio, sr_ratios=sr_ratios, depths=depths, linear=True) for i in range(n)])
...
class WavePVTv2(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.WavePVTLayerv2(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
                                   mlp_ratio=mlp_ratio, sr_ratios=sr_ratios, depths=depths, linear=True) for i in range(n)])
...
class WavePVTv3(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.WavePVTLayerv3(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
                                   mlp_ratio=mlp_ratio, sr_ratios=sr_ratios, depths=depths, linear=True) for i in range(n)])
...
class WavePVTv4(nn.Module):
    def __init__(self, c1, c2, patch_size, stride, mlp_ratio, sr_ratios, depths, n=1):
        super().__init__()
        num_heads = c2//64
        self.blocks = nn.Sequential(*[PVT.WavePVTLayerv4(in_chans=c1, embed_dims=c2, patch_size=patch_size, stride=stride, num_heads=num_heads, 
                                   mlp_ratio=mlp_ratio, sr_ratios=sr_ratios, depths=depths, linear=True) for i in range(n)])
